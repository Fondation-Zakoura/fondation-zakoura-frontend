...
const EditProject: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (!form.project_bank_account_id) newErrors.project_bank_account_id = "Compte bancaire requis";
    ...
    <div className="p-8 font-nunito">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
              <label ref={inputRefs.responsible} className="block text-gray-700 font-semibold mb-2 text-left">Responsable</label>
              <Combobox
                  ...
                  <div className="flex-1 flex flex-col md:flex-row gap-4">
                    <div className="flex-1">
                      ...
                      <Combobox
                        ...
                        disabled={optionsLoading}
                      />
                      {partnerErrors[idx]?.partner_id && <div className="text-red-500 text-xs mt-1">{partnerErrors[idx].partner_id}</div>}
                    </div>
                    <div className="flex-1">
                      ...
                      <Combobox
                        ...
                        disabled={optionsLoading}
                      />
                      {partnerErrors[idx]?.partner_role && <div className="text-red-500 text-xs mt-1">{partnerErrors[idx].partner_role}</div>}
                    </div>
                    <div className="flex-1">
                      ...
                      <Input
                        ...
                        className="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition"
                      />
                      {partnerErrors[idx]?.partner_contribution && <div className="text-red-500 text-xs mt-1">{partnerErrors[idx].partner_contribution}</div>}
                    </div>
                  </div>
                  {partners.length > 0 && (
                    <Button
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent>
          <DialogHeader>
            <div className="text-lg font-semibold text-red-500">Alerte budget</div>
          </DialogHeader>
          <div className="py-4 text-gray-800 text-center">{modalMessage}</div>
          <DialogFooter className="flex justify-end gap-4">
            <Button variant="outline" onClick={handleModalCancel} type="button">Annuler</Button>
            <Button onClick={handleModalConfirm} type="button" className="bg-red-500 hover:bg-red-600 text-white">Confirmer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {/* Show the sum of percentages with color */}
          <div className={`mt-2 text-center text-lg font-bold ${getTotalPercent().sum < 100 ? 'text-red-600' : getTotalPercent().sum > 100 ? 'text-green-600' : 'text-gray-700'}`}>
            Total contributions: {getTotalPercent().sum.toFixed(2)}%
          </div>
      </Card>
    </div>
