...
import { useGetProjectBankAccountsQuery, useCreateProjectBankAccountMutation, useUpdateProjectBankAccountMutation, useDeleteProjectBankAccountMutation, useUpdateBankAccountSupportingDocumentMutation } from '@/features/api/projectsApi';
...
const emptyAccount: Omit<ProjectBankAccount, 'supporting_document'> & { supporting_document: string | File } = {
  id: 0,
  rib_iban: '',
  agency: '',
  bank: '',
...
const ProjectBankAccountsPage: React.FC = () => {
  ...
  const [updateBankAccountSupportingDocument] = useUpdateBankAccountSupportingDocumentMutation();

  const [modal, setModal] = useState<'add' | 'edit' | 'show' | null>(null);
  const [selected, setSelected] = useState<ProjectBankAccount | null>(null);
  const [form, setForm] = useState<FormType>({ ...emptyAccount, supporting_document: '' });
  ...
  const openEdit = (account: ProjectBankAccount) => {
    setForm({ ...account, status: normalizeStatus(account.status), supporting_document: account.supporting_document || '' });
    setSelected(account);
    setModal('edit');
    setFilePreview(null);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.type === 'file') {
      updatedForm = { ...form, [e.target.name]: e.target.files && e.target.files[0] ? e.target.files[0] : '' };
    } else {
  ...
  function validateForm(form: FormType) {
    ...
    const requiredFields: (keyof FormType)[] = [
      'rib_iban', 'agency', 'bank', 'account_title', 'opening_country', 'account_holder_name', 'bic_swift', 'currency', 'status', 
    ];
    ...
    // rib_iban length
    if (form.rib_iban && (form.rib_iban.length < 24 || form.rib_iban.length > 34)) {
      errors.rib_iban = 'Le RIB/IBAN doit contenir entre 24 et 34 caractères.';
    }
    // bic_swift format
    if (form.bic_swift && !/^[A-Za-z0-9]{8}(?:[A-Za-z0-9]{3})?$/.test(form.bic_swift)) {
      errors.bic_swift = 'Le BIC/SWIFT doit contenir 8 ou 11 caractères alphanumériques.';
    }
    // supporting_document type and size
    if (form.supporting_document && typeof form.supporting_document !== 'string') {
      ...
      if (!allowedTypes.includes(file.type)) {
        errors.supporting_document = 'Le fichier doit être un PDF ou une image.';
      }
      if (file.size > 5 * 1024 * 1024) {
        errors.supporting_document = 'La taille maximale du fichier est de 5 Mo.';
      }
    }
    return errors;
  }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isFile = form.supporting_document && typeof form.supporting_document !== 'string';

      let payload: any;
      if (isFile) {
        payload = new FormData();
        Object.entries(form).forEach(([key, value]) => {
          if (key === 'supporting_document' && value && typeof value !== 'string') {
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    if (!selected) { setEditLoading(false); return; }
    try {
      console.log('Submitting form:', form);
      const isFile = form.supporting_document && typeof form.supporting_document !== 'string';
      ...
      if (isFile) {
        ...
        formData.append('supporting_document', form.supporting_document as File);
  ...
  const columnFilters = useMemo((): ColumnFilter<ProjectBankAccount>[] => {
    const uniqueRibs = Array.from(new Set(accounts.map((a: { rib_iban: any; }) => a.rib_iban)));
    const uniqueAgencies = Array.from(new Set(accounts.map((a: { agency: any; }) => a.agency)));
    const uniqueBanks = Array.from(new Set(accounts.map((a: { bank: any; }) => a.bank)));
  ...
  const columns: Column<ProjectBankAccount>[] = [
    ...
    { key: "opening_country", header: "Pays d'ouverture" ,sortable:true},
    {
      ...
      header: "Status",
      render: (row) => (
        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold shadow ${getStatusBadgeClass(row.status)}`}>
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl shadow p-6">
        {isLoading ? (
          ...
          data={accounts}
          hoverEffect
          emptyText={isLoading ? 'Chargement des données...'  : 'Aucun Compte bancaire trouvé'}
          headerStyle={'primary'}
          striped
          initialPageSize={10}
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <Input id="rib_iban" name="rib_iban" placeholder="RIB / IBAN" value={form.rib_iban} onChange={handleChange} required />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="agency" name="agency" placeholder="Agence" value={form.agency} onChange={handleChange} required />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Select value={form.bank} onValueChange={value => setForm({ ...form, bank: value })}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="account_title" name="account_title" placeholder="Intitulé du compte" value={form.account_title} onChange={handleChange} required />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="account_holder_name" name="account_holder_name" placeholder="Nom du titulaire" value={form.account_holder_name} onChange={handleChange} required />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="bic_swift" name="bic_swift" placeholder="BIC / SWIFT" value={form.bic_swift} onChange={handleChange} />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="opening_date" type="date" name="opening_date" placeholder="Date d'ouverture" value={form.opening_date} onChange={handleChange} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="opening_country">Pays d'ouverture <span className="text-red-500">*</span></Label>
                <Input id="opening_country" type="text" name="opening_country" placeholder="Pays d'ouverture" value={form.opening_country} onChange={handleChange} required />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input id="supporting_document" name="supporting_document" type="file" accept="application/pdf,image/*" onChange={handleChange} />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Textarea id="comments" name="comments" placeholder="Commentaires / Remarques" value={form.comments} onChange={handleTextareaChange} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                {!filePreview && typeof form.supporting_document === 'string' && form.supporting_document && !removeExistingFile && (
                  <div className="relative flex items-center gap-2 mb-2">
                    {/\.(jpg|jpeg|png|gif|bmp|webp|svg)$/i.test(form.supporting_document) ? (
                      <img
