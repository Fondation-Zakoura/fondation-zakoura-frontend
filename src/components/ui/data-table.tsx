...
export function DataTable<T extends object>({
  ...
  // --- STYLE HELPERS ---
  const getRowHeightClass = () => ({ sm: "h-10", md: "h-12", lg: "h-14" })[rowHeight] || "h-12";
  const getHeaderStyleClass = () => ({ dark: "bg-gray-800 text-white", primary: "bg-[#008c95] text-white" })[headerStyle] || "bg-gray-50 text-gray-700";
        ...
        <div className="flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="w-full md:w-1/3 lg:w-1/4">
            ...
            <div className="relative">
              ...
              <Input id="global-search" placeholder="Rechercher..." value={globalFilter} onChange={handleGlobalFilterChange} className="pl-10"/>
            </div>
          </div>
          ...
          {columnFilters.map((filter) => (
            <div key={String(filter.id)} className="w-full md:w-auto md:min-w-[180px]">
              <label htmlFor={`filter-${String(filter.id)}`} className="block text-sm font-medium text-gray-700 mb-1">
                {filter.label}
              </label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all">{`Tout ${filter.label.split(' ')[0].toLowerCase()}`}</SelectItem>
          ...
          <div className="w-full md:w-auto md:ml-auto pt-0 md:pt-5">
            <Button variant="outline" onClick={handleResetFilters} className="w-full flex items-center justify-center gap-2 text-gray-700">
        ...
        <Table className="min-w-full">
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={columns.length} className="text-center py-16 bg-gray-50">
                  <div className="flex flex-col items-center justify-center text-gray-400">
                    <div className="bg-gray-200 rounded-full p-3 mb-3">
                      <Search className="h-8 w-8" />
                    </div>
                    <p className="text-lg font-medium">{emptyText}</p>
                    <p className="mt-1 text-sm max-w-md">Try adjusting your search or filter to find what you're looking for</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
