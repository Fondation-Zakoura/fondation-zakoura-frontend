      ...
      <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:flex-wrap items-center gap-4"> {/* Added md:flex-wrap */}
          ...
          {/* Mapped Dropdown Filters */}
            {/* Mapped Filters (dropdown or text input) */}
          {columnFilters.map((filter) => (
            <div key={filter.id} className="w-full md:w-auto md:min-w-[180px] flex-grow">
              <label htmlFor={`filter-${filter.id}`} className="block text-sm font-medium text-gray-700 mb-1 text-left">
                {filter.label}
              </label>
              ...
              {filter.options && filter.options.length > 0 ? (
                <Select
                  ...
                  onValueChange={v => handleFilterValueChange(filter.id, v)}
            >
        <SelectTrigger id={`filter-${filter.id}`} className="w-full">
          <SelectValue placeholder={`Tout ${filter.label.toLowerCase()}`} />
        </SelectTrigger>
        <SelectContent>
